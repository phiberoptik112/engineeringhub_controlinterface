...
- `journaler monitor` renders live daemon status from HTTP `/status` plus `.journaler/daemon_status.json`, `state.json`, and `context_cache.json`; `journaler.activity_log` can append daemon events/suggestions to today's journal or a dedicated org file for Doom Emacs auto-revert workflows.
